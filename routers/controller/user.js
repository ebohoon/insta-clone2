  ...
  try {
    const UserSchema = Joi.object({
      ...
      nickname: Joi.string()
        ...
        .pattern(/^[a-z0-9]{3,10}/)
        .required(),
      password: Joi.string().min(4).pattern(new RegExp("^[a-z0-9]{4,10}$")).required(),
      confirmPassword: Joi.ref("password"),
    })

    const { userId, nickname, password, confirmPassword } = await UserSchema.validateAsync(req.body)
    const CheckBody = await CheckRegister(userId, nickname, password, confirmPassword)
    ...
    if (await Users.findOne({ userId })) {
      ...
      const Encryptpassword = bcrypt.hashSync(password, parseInt(process.env.SALT))
